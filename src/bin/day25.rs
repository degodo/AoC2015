...
//! 
//! Lösung:
//! Das Element in Zeile n Spalte 1 (n,1) hat die Ordmungszahl (n-1)*n/2+1.
//! Da ist die Summe über eine Folge 1,2,3,....
//! Die Element in Spalte 2 der selben Zeile hat die Ordnungszahlt des ersten
//! Elements (n+1,1) plus 1.
//! Das Elemte in Spalte 3 der Zeile n die Odrnungszahlt des ersetn Elemntes
//! (n+2,1) plus 2.
//! Daraus ergit sie die Ordnungszahl für Zeile z und Spalte s zu:
//! (z + s - 2) * (r + s - 1) / 2 + s  

fn calc(seed: u32, n: u32) -> u32 {
    let mut result: u64 =  seed as u64;
    for _i in 2..=n {
...
fn main() {
    const ROW: u32 = 2947;
    const COLUMN: u32 = 3029;
    let n = (ROW+COLUMN-2)*(ROW+COLUMN-1)/2+COLUMN;
    println!("{} {}", n, calc(20151125, n));
} 
